...
ts175 = ts( tam_of_clstrs['clstr_175'], start=1, end=90, frequency = 24 )
...
ts_strt = 43 # use same day on next week for testing.
ts_stop = 43
...
ts175_train_d = decompose(ts175_train)
plot(ts175_train_d)


new_d = decompose(ts175_train_d$random)
new_d1 = decompose(new_d$random)



df = data.frame(seq(1,985),ts175_train_d$seasonal, ts175_train_d$trend)

colnames(df)

fig = plot_ly(df,x=~seq(1,985), y=~(ts175_train_d$seasonal +ts175_train_d$trend + new_d$trend + new_d$seasonal + new_d1$trend + new_d1$seasonal) , type='scatter', mode='lines')%>% 
  add_trace(y=ts175_train)
fig 


new_tr_train = ts175_train_d$seasonal +ts175_train_d$trend + new_d$trend + new_d$seasonal + new_d1$trend + new_d1$seasonal

new_tr_train = new_tr_train[37:949]

######## ACF plot ###### 
ts175_tr_acf = acf(new_tr_train,pl=FALSE,lag.max=length(new_tr_train))
...
######## PACF plot ###
ts175_tr_pacf = pacf(new_tr_train,pl=FALSE,lag.max=length(new_tr_train))
...
for(r_acf in seq(1:5))
{
  for(r_pacf in seq(1:2))
  {
    ...
    ts175_train_fit = tscount::tsglm(ts=new_tr_train, link='log', model =
    ...
    data = data.frame(seq(1:length(new_tr_train)), new_tr_train, fitted.values(ts175_train_fit))
...
ts175_train_fit_TRUE2 = tscount::tsglm(ts=ts175_train, link='log', model =
                                         list(past_obs = true_reg_lags2, past_mean = true_reg_lags2),distr = 'nbinom')
...
ts175_train_fit_TRUE3 = tscount::tsglm(ts=ts175_train, link='log', model =
                                         list(past_obs = true_reg_lags3, past_mean = true_reg_lags3),distr = 'nbinom')
...
ts175_train_fit_TRUE4 = tscount::tsglm(ts=ts175_train, link='log', model =
                                         list(past_obs = true_reg_lags4, past_mean = true_reg_lags4),distr = 'nbinom')
...
ts175_train_fit_TRUE5 = tscount::tsglm(ts=ts175_train, link='log', model =
                                         list(past_obs = true_reg_lags5, past_mean = true_reg_lags5),distr = 'nbinom')
...
ts175_train_fit_TRUE6 = tscount::tsglm(ts=ts175_train, link='log', model =
                                         list(past_obs = true_reg_lags6, past_mean = true_reg_lags6),distr = 'nbinom')
...
test_sum_abs_err = sum(abs(as.vector(ts175_test) - floor(0.5+ts175_test_fit)))
test_sum_abs_err
