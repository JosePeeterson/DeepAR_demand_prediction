...
"""Linux"""
os.chdir("/home/optimusprime/Desktop/peeterson/github/DeepAR_demand_prediction/2_freq_nbinom_LSTM/1_cluster_demand_prediction/data/demand_data/standalone/country_level/train_val_test_data/walk_forward_validation")
#os.chdir("/home/optimusprime/Desktop/peeterson/github/DeepAR_demand_prediction/2_freq_nbinom_LSTM/1_cluster_demand_prediction/data/demand_data/standalone/country_level/train_val_test_data/walk_forward_validation")
...
fewer_clstr_train1_dem_data = pd.read_csv(region+'_fewer_clstr_train1.csv')
fewer_clstr_train2_dem_data = pd.read_csv(region+'_fewer_clstr_train2.csv')
fewer_clstr_train3_dem_data = pd.read_csv(region+'_fewer_clstr_train3.csv')
fewer_clstr_full_train_dem_data = pd.read_csv(region+'_fewer_clstr_full_train.csv')


fewer_clstr_val1_dem_data = pd.read_csv(region+'_fewer_clstr_val1.csv')
fewer_clstr_val2_dem_data = pd.read_csv(region+'_fewer_clstr_val2.csv')
fewer_clstr_val3_dem_data = pd.read_csv(region+'_fewer_clstr_val3.csv')

fewer_clstr_test_dem_data = pd.read_csv(region+'_fewer_clstr_test.csv')
fewer_clstr_all_dem_data  = pd.read_csv(region+'_fewer_clstr_all.csv')


train_data_list = [fewer_clstr_train1_dem_data, fewer_clstr_train2_dem_data, fewer_clstr_train3_dem_data]
full_train_data = fewer_clstr_full_train_dem_data
val_data_list = [fewer_clstr_val1_dem_data,fewer_clstr_val2_dem_data,fewer_clstr_val3_dem_data]
test_data = fewer_clstr_test_dem_data
all_data = fewer_clstr_all_dem_data
###### fewer number of clusters to train faster ########
...
print("full train_data has null values?",full_train_data.isnull().values.any())
#################### add date information ts ####################
...
all_data['_hour_of_day'] = all_data["datetime"].dt.hour.astype(str)
all_data['_day_of_week'] = all_data["datetime"].dt.dayofweek.astype(str)
all_data['_day_of_month'] = all_data["datetime"].dt.day.astype(str)
all_data['_day_of_year'] = all_data["datetime"].dt.dayofyear.astype(str)
all_data['_week_of_year'] = all_data["datetime"].dt.weekofyear.astype(str)
all_data['_month_of_year'] = all_data["datetime"].dt.month.astype(str)
all_data['_year'] = all_data["datetime"].dt.year.astype(str)
print("all_data has null values?",all_data.isnull().values.any())
#################### add date information ts ####################
...
fewer_clstr_train_dem_data = pd.read_csv(region+'_fewer_clstr_train_dem_data1.csv')
fewer_clstr_val_dem_data = pd.read_csv(region+'_fewer_clstr_val_dem_data1.csv')
...
all_datetime = all_data['datetime']
...
for train_dat in train_data_list:
    train_dat = train_dat.drop(columns=['datetime'])

for val_dat in val_data_list:
    val_dat = val_dat.drop(columns=['datetime'])
...
def objective(trial,):  
    ...
    neu = trial.suggest_int(name="neu",low=900,high=1200,step=50,log=False)
    lay = trial.suggest_int(name="lay",low=2,high=4,step=1,log=False)
    bat = batch#trial.suggest_int(name="bat",low=4,high=8,step=4,log=False)
    lr = trial.suggest_float(name="lr",low=0.0001,high=0.01,log=True)
    num_ep = total_num_epochs# trial.suggest_int(name="num_ep",low=25,high=40,step=2,log=False)
    enc_len = trial.suggest_int(name="enc_len",low=12,high=18,step=2,log=False)
    #enc_len = encoder_length
    pred_len = prediction_length
    drop = trial.suggest_float(name="dropout",low=0.1,high=0.3,step=0.1,log=False)
    print(f'This trial parameters, neu: {neu}, lay: {lay}, bat: {bat}, lr: {lr}, enc_len: {enc_len}, drop: {drop}')

    val_loss_folds = []
    val_loss_epoch_folds = []
    pas = 0
    for train_data,val_data in zip(train_data_list,val_data_list):
        ...
        val_data['time_idx'] = val_data['time_idx'].astype(int)
        ...
        val_loss_folds.append(min_val_loss)
        val_loss_epoch_folds.append(min_val_loss_epoch)
    ...
    global_min_val_loss_epoch = int(np.mean(np.array(val_loss_epoch_folds)))
    global_min_val_loss = np.mean(np.array(val_loss_folds))
    trial.report(global_min_val_loss, global_min_val_loss_epoch)
    ...
    return global_min_val_loss #min_val_rmse
...
study.optimize(objective, timeout=None, n_trials=2)
...
batch_size = batch#study.best_params["bat"]
learning_rate = study.best_params["lr"]
dropout = study.best_params["dropout"]
encoder_length = study.best_params["enc_len"]
max_epochs = total_num_epochs#study.best_params["num_ep"]
...
for neu,lay,bat,lr,enc_len,pred_len,drop,num_ep in product(*[x for x in hparams_grid.values()]):
    ...
    #all_dataset = TimeSeriesDataSet.from_dataset(full_train_dataset,all_data, stop_randomization=True)
    ...
    #all_dataloader = all_dataset.to_dataloader(train=False, )    
    ######### Load DATA #############
    ...
    #all_output = deepar.predict(data=all_dataloader,mode='prediction',return_index=True,num_workers=8,show_progress_bar=True) # for all dataset
    #test_output = all_output # for all dataset
    #test_data = all_data # for all dataset

    actual = {}
    prediction = {}

    with PdfPages(region+'_results_bias1_cross_val.pdf') as pdf:
        ...
        for grp in groups_list:
            ...
            xy = np.vstack([predictions_grp_list,actuals_grp_list])
            z = gaussian_kde(xy)(xy)
            fig2, ax = plt.subplots()
            cax = ax.scatter(predictions_grp_list, actuals_grp_list, c=z, s=100)
            fig2.colorbar(cax)
            ...
            # TODO: ACF of Residuals
            fig3, ax = plt.subplots()
            acf = smt.graphics.plot_acf( np.array(actuals_grp_list)- np.array(predictions_grp_list)  ,lags=len(actuals_grp_list)-1,alpha=0.05,)
            plt.title('ACF Residuals - all')
            pdf.savefig() 
            plt.close()
            fig3a, ax = plt.subplots()
            acf1 = smt.graphics.plot_acf( np.array(actuals_grp_list)- np.array(predictions_grp_list)  ,lags=48,alpha=0.05,)
            plt.title('ACF Residuals - 48 lags')
            pdf.savefig() 
            plt.close()
            ...
            #print(f'\nDEEPAR: PRECISION, RECALL & F1 scores for cluster: {grp} = \n{classification_report(actuals_grp_list,predictions_grp_list)}\n')
        ...
        print(f'\nDEEPAR: PRECISION, RECALL & F1 scores for Region: {region} = \n{classification_report(total_actual_list,total_prediction_list)}\n')
    ...
    # test_data['datetime'] = all_datetime # for all dataset
    # entire_data = test_data  # for all dataset
    # season_len = 1 # for all dataset
    # num_past_seas = 2 # for all dataset
    ...
    for grp in groups_list:
        ...
        #print(f'\nHA: PRECISION, RECALL & F1 scores for cluster: {grp} = \n{classification_report(actuals_grp_list,predictions_grp_list)}\n')
    ...
    print(f'\HA: PRECISION, RECALL & F1 scores for Region: {region} = \n{classification_report(tot_actual_flat_arr,tot_prediction_flat_arr)}\n')
...
with open(region+"_fewer_clstrs_bias1_cross_val.json", "w") as outfile:
