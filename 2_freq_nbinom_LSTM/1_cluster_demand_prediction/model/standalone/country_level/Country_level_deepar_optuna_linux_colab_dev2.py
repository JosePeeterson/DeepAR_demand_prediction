...
batch = 4
prediction_length = 1
filename = "_fewer_clstrs_hist_avg_fut_inf_all_data"
#encoder_length = 18
######### MISCELLANEOUS ###################
...
fewer_clstr_full_train_dem_data = pd.read_csv(region+'_fewer_clstr_full_train_dem_data1.csv')
fewer_clstr_val_dem_data = pd.read_csv(region+'_fewer_clstr_val_dem_data1.csv')
fewer_clstr_test_dem_data = pd.read_csv(region+'_fewer_clstr_test_dem_data1.csv')
fewer_clstr_all_dem_data = pd.read_csv(region+'_fewer_clstr_all_dem_data1.csv')
...
def objective(trial,):  
    ...
    neu = trial.suggest_int(name="neu",low=1000,high=1400,step=100,log=False)
    ...
    enc_len = trial.suggest_int(name="enc_len",low=16,high=22,step=2,log=False)
    #enc_len = encoder_length
    pred_len = prediction_length
    drop = trial.suggest_float(name="dropout",low=0.1,high=0.4,step=0.1,log=False)
...
# study.optimize(objective, timeout=None, n_trials=150)
...
neurons = 900#study.best_params["neu"]
layers = 4#study.best_params["lay"]
batch_size = batch#study.best_params["bat"]
learning_rate = 0.00111066589#study.best_params["lr"]
dropout = 0.3#study.best_params["dropout"]
encoder_length = 16#study.best_params["enc_len"]
...
for neu,lay,bat,lr,enc_len,pred_len,drop,num_ep in product(*[x for x in hparams_grid.values()]):
    ...
    all_dataset = TimeSeriesDataSet.from_dataset(full_train_dataset,all_data, stop_randomization=True) #for all dataset   
    ...
    all_dataloader = all_dataset.to_dataloader(train=False, ) #for all dataset   
    ######### Load DATA #############
    ...
    #     quantile_values = raw_predictions['prediction'][t,grp,:].quantile(quantiles) 
    #     quantile_10 = quantile_values[..., 0]
    #     quantile_90 = quantile_values[..., 1]
    # ############ Quantile Forecast ##############
    ...
    with PdfPages(region+filename+'.pdf') as pdf:
...
with open(region+filename+".json", "w") as outfile:
